...
### ⚠️ MeshPart를 만들면 OVERDARE가 `Size`를 덮어쓴다

`instance.create` 로 `meshId` 를 가진 MeshPart를 만들면, Studio가 로드하면서 요청한
`Size` 를 **메시 원본 바운즈로 갈아치운다.** 배율은 메시마다 달라서 1.3배부터 131배까지
나왔다 (감시탑 625×1200×569 → 25785×157903×22873, 즉 1.6km). 맵 전체가 킬로미터 단위로
부풀어 카메라가 어디에 있든 지오메트리 안에 갇힌다.

`UnitExtent` 는 요청한 절반값 그대로 남으므로 **`UnitExtent` 만 보고 "정상"이라 판단하면 안 된다** —
실제 렌더는 `Size` 를 따른다.

**대처**: 인스턴스를 만들고 Studio가 저장한 뒤, `.ovdrjm` 의 각 노드 `Size` 를 의도한 값으로
다시 써넣고 `level.apply` 한다. 사후에 쓴 값은 유지된다. `overdare_update_instance` 로
개별 수정해도 먹지만, 수백 개면 파일을 직접 고치는 편이 한 번에 끝난다.
